...
Fireworks.BindTriggerDomEvents = function (emitter, domElement) {
	...
	onMousedown = function () { emitter.effect('spawner').opts.start(); };
	this._onMouseUp = function () { emitter.effect('spawner').opts.stop(); };
	onMouseup = function () { emitter.effect('spawner').opts.stop(); };
...
loadTremulousParticles(urls, function (images) {
	...
	var emitter = Fireworks.createEmitter({ nParticles: 200 })
		...
		.acceleration({
			effectId: 'gravity',
			shape: Fireworks.createShapePoint(0, 0, 0)
		})
		.lifeTime(1.5)
		...
		.onRender(function (particle) {
			...
			var positionX = position.x - width * 2.25;
